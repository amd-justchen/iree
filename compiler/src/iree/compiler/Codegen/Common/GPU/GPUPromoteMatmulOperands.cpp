...
Value promoteValue(OpBuilder &builder, Location loc, Value v) {
  ...
  if (clUseDirectLoad) {
    //IREE::Codegen::LoweringConfigTilingLevelsAttr tilingLevelsAttr =
    //    IREE::Codegen::LoweringConfigTilingLevelsAttr::get(
    //        builder.getContext(), IREE::GPU::TilingLevel::Subgroup);
    auto globalLoadDMAAttr = IREE::GPU::UseGlobalLoadDMAAttr::get(
        builder.getContext());
